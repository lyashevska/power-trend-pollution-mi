##---------------------------
## function to detect trend in historical data
## no change in slope
...
f_detect_trend_hist <- function(nobs_year, tailyrs = NULL, data, mod) {
  ## nobs_year: the number of observations per year
  ## yrs: how many years of data are used, default all
  ## data: data  
  ## mod: model
  ...
  if (is.null(tailyrs)) {
    ...
    # take last n years
    years <- tail(min(data$Year):max(data$Year), n = tailyrs)
  }
  ...
  # was significant trend with correct slope detected
