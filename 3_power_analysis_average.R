##---------------------------
## simulations for average assessment area pollution power analysis
## title: addresses issue #2
## data: juvenile HP
## OL, CM: 30/1/26
##---------------------------
...
## calculate the inputs needed per 
aa_summary <- data.frame(id = ids, mu_rbw = NA , sd_rbw = NA, min_lat = NA, max_lat = NA, res_sd = NA)

for (i in seq_along(ids)) {
    print(ids[i])
    temp <- data[data$Ospar.AA == ids[i],]
    ##temp$Year <- temp$Year - min(temp$Year)
    aa_summary$mu_rbw[i] <- mean(temp$Rel.body.wt)
    aa_summary$sd_rbw[i] <- sd(temp$Rel.body.wt)
    aa_summary$min_lat[i] <- range(temp$Latitude)[1]
    aa_summary$max_lat[i] <- range(temp$Latitude)[2]
    ## made it to here 
}  
...
# load functions
source("f_detect_trend_average.R")
